...
CHECKBOX_MAP = {
    ...
    # E6.1: Scope 3 categories — boxes numbered column-first:
    # left col (boxes 1-9, top→bottom) then right col (boxes 10-17, top→bottom)
    'E6.1': {
        'fields': {
            # Left column (boxes 1-9)
            'purchased goods and services':                    'E6.1 Check Box 1',
            'capital goods':                                   'E6.1 Check Box 2',
            'fuel- and energy-related activities':             'E6.1 Check Box 3',
            'upstream transportation and distribution':        'E6.1 Check Box 4',
            'waste generated in operations':                   'E6.1 Check Box 5',
            'business travel':                                 'E6.1 Check Box 6',
            'employee commuting':                              'E6.1 Check Box 7',
            'upstream leased assets':                          'E6.1 Check Box 8',
            'downstream transportation and distribution':      'E6.1 Check Box 9',
            # Right column (boxes 10-17)
            'processing of sold products':                     'E6.1 Check Box 10',
            'use of sold products':                            'E6.1 Check Box 11',
            'end-of-life treatment of sold products':          'E6.1 Check Box 12',
            'downstream leased assets':                        'E6.1 Check Box 13',
            'franchises':                                      'E6.1 Check Box 14',
            'investments':                                     'E6.1 Check Box 15',
            'other - upstream':                                'E6.1 Check Box 16',
...
def debug_dry_run(company_name):
    ...
    try:
        ...
        for s in clean:
            ...
            if qid in CHECKBOX_MAP:
                ...
                for opt_norm, fname in q['fields'].items():
                    ...
                    if m:
                        field_values[fname] = '/Yes'   # mirror fill_pdf logic
        ...
        return jsonify({
            ...
            'field_values_checkboxes': {k: v for k, v in field_values.items() if 'Check Box' in k},
            'g10_g11_rows': [
...
@app.route('/api/test-g4-1', methods=['GET'])
def test_g4_1():
    """Debug: fill only G5.1 Radio Button fields (G4.1 visual section) with Yes on row 1, No on rows 2-4."""
    try:
        reader = PdfReader(PDF_PATH)
        writer = PdfWriter()
        writer.append(reader)
        # kid 0 = Yes, kid 1 = No
        radio_values = {
            'G5.1 Radio Button 1': 0,   # row 1 = Yes
            'G5.1 Radio Button 2': 1,   # row 2 = No
            'G5.1 Radio Button 3': 1,   # row 3 = No
            'G5.1 Radio Button 4': 1,   # row 4 = No
        }
        radio_on_states = _set_radio_fields(writer, radio_values)
        from pypdf.generic import NameObject as _NameObj, TextStringObject as _TSO
        for page in writer.pages:
            ...
            for annot_ref in annots:
                ...
                if opt_ref is not None:
                    try:
                        ...
                        opt_list = list(opt)
                        if 0 <= target_idx < len(opt_list):
                            try: export_str = opt_list[target_idx].get_data().decode('utf-8', errors='replace')
                            except Exception: export_str = str(opt_list[target_idx]).strip('()')
                            parent[_NameObj('/V')] = _TSO(export_str)
                        else:
                            parent[_NameObj('/V')] = _NameObj(on_state)
                    except Exception:
        ...
        buf.seek(0)
        return Response(buf.read(), mimetype='application/pdf',
                        headers={'Content-Disposition': 'attachment; filename="test_g4_1.pdf"'})
    except Exception as e:
        import traceback
        return jsonify({'error': str(e), 'trace': traceback.format_exc()}), 500
